...
function EditContact() {
  ...
  const fetchContact = async () => {
    try {
      ...
      setContact({
        ...
        country: contactData.country || '+91',
      ...
      if (contactData.photo) {
        setPhotoPreview(`${BACKEND_URL}${contactData.photo}`);
      }
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.put(`${BACKEND_URL}/contacts/${id}`, formData, {
        headers: {
        ...
        <div className="col-md-8 col-lg-6">
          <div className="card shadow-lg">
            <div className="card-body p-5">
              <h1 className="card-title text-center mb-4">Edit Contact</h1>
              <form onSubmit={handleSubmit} className="edit-contact-form">
                <div className="mb-3">
                  <div className="photo_container d-flex justify-content-center">
                    <div 
                      className="photo-upload me-3 rounded-circle overflow-hidden" 
                      style={{width: '100px', height: '100px', cursor: 'pointer'}}
                      onClick={() => document.getElementById('photoUpload').click()}
                    >
                      {photoPreview ? (
                        <img src={photoPreview} alt="Contact" className="w-100 h-100 object-fit-cover" />
                      ) : (
                ...
                {!showMore && (
                  <button 
                    type="button" 
                    className="btn btn-link w-100 text-center text-dark p-0" 
                ...
                <button type="submit" className="btn btn-primary w-100 mt-3" disabled={!!phoneError}>
                  Update Contact
                </button>
              </form>
            </div>
